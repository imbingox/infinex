...
- Control Plane、backtest worker 与 live worker 分别通过短格式 bind mount `${INFINEX_DATA_ROOT}/control-plane`、`${INFINEX_DATA_ROOT}/backtest-worker`、`${INFINEX_DATA_ROOT}/live-worker`。目录不存在时由 Docker 自动创建；迁移时停机复制对应目录。
